                                                  ...
                                                  size_t len )
{
    ...
    while( list_size > 0 )
    {
            ...
            p[0] == POLARSSL_ECP_PF_COMPRESSED )
        {
            ...
            ssl->handshake->point_format = p[0];
...
static int ssl_write_client_key_exchange( ssl_context *ssl )
{
    ...
    if( ssl_ciphersuite_is_dh_pkcsign( ciphersuite_info->key_exchange ) )
    {
        ...
        ret = ssl->handshake->dhif_info->write_premaster(
                ...
                /* WTF buffer size should be enough */
        ...
        ciphersuite_info->key_exchange == POLARSSL_KEY_EXCHANGE_ECDHE_PSK )
    {
        ...
        else if( ssl_ciphersuite_is_dh_psk( ciphersuite_info->key_exchange ) )
        {
            ret = ssl->handshake->dhif_info->gen_public(
                    ssl->handshake->dhif_ctx,
                    ssl->f_rng, ssl->p_rng );
