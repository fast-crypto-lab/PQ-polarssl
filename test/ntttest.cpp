...
*/
	...
	for(int i=0;i<n;i++){
		mpz_powm_ui(a.get_mpz_t(), w.get_mpz_t(), i,  q.get_mpz_t());
		data1[i]=data1[i]*a%q;
	}
...
//	data2[0]=1;
	...
	fntt(data1, n, t, q, NTTW_FORWARD);
	fntt(data2, n, t, q, NTTW_FORWARD);
...
*/
	...
	fntt(data1, n,t, q, NTTW_INVERSE);

	mpz_class invN=n;
	mpz_invert(invN.get_mpz_t(),invN.get_mpz_t(),q.get_mpz_t());

	ntt_norm(data1, n, invN,q );

	mpz_invert(w.get_mpz_t(),w.get_mpz_t(),q.get_mpz_t());
...
cout<<"generate a:"<<endl;
	gmp_randstate_t rnds;
	gmp_randinit_default(rnds);
	gmp_randseed_ui (rnds, 0);
	for(int i=0;i<n;i++){
		mpz_urandomm (t.get_mpz_t(), rnds , q.get_mpz_t());
		cout<<"	mpi_read_string( ctx->a->a["<<i<<"], 10, \""<<t<<"\" ); "<<endl;
